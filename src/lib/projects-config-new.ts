...
// Content data - kept separate from metadata for clarity
...
export const buildProjectConfig = (slug: string): ProjectConfig => {
  const projectMeta = metadata.projects.find(p => p.slug === slug);
  const projectContent = projectContent[slug as keyof typeof projectContent];

  if (!projectMeta || !projectContent) {
    return buildProjectConfig('vida');
  }

  const imageStrategy = {
    placement: projectMeta.images && Object.keys(projectMeta.images).length > 0 ? 'alternating' : 'none',
    rationaleImages: (() => {
      const images: Record<number, string[]> = {};
      let count = 1;
      while (projectMeta.images[`rationale${count as any}`]) {
        images[count - 1] = [projectMeta.images[`rationale${count as any}`]];
        count++;
      }
      return images;
    })(),
    inlineContent: projectMeta.inlineContent,
  };
  ...
  return {
    ...
    ...projectContent,
    imageStrategy,
    imagePrefix: projectMeta.images.hero?.split('-').slice(0, -1).join('-') || projectMeta.slug,
    showHeroImage: !projectMeta.hasNoImages,
  } as ProjectConfig;
};

export const getProjectConfig = (slug: string): ProjectConfig => buildProjectConfig(slug);
